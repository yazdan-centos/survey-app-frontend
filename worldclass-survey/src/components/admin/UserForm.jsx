...
const inputClass = 'w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-primary-700 focus:outline-none focus:ring-1 focus:ring-primary-700';
...
export default function UserForm({ user, submitting, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-5" aria-labelledby="user-form-title">
      ...
      <fieldset disabled={submitting} className="min-w-0 space-y-4 disabled:opacity-60">
        <div>
          <label htmlFor="user-username" className="mb-1 block text-xs font-medium text-slate-600">نام کاربری *</label>
        ...
        <div>
          <label htmlFor="user-full-name" className="mb-1 block text-xs font-medium text-slate-600">نام و نام خانوادگی</label>
          <input id="user-full-name" name="fullName" value={values.fullName} onChange={change} autoComplete="off" className={inputClass} />
        </div>
        <div>
          <label htmlFor="user-email" className="mb-1 block text-xs font-medium text-slate-600">ایمیل</label>
        ...
        <div>
          <label htmlFor="user-password" className="mb-1 block text-xs font-medium text-slate-600">{user ? 'رمز عبور جدید (اختیاری)' : 'رمز عبور *'}</label>
          ...
          {user && <p id="user-password-help" className="mt-1 text-xs text-slate-500">برای حفظ رمز عبور فعلی، این فیلد را خالی بگذارید.</p>}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" onClick={onCancel} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600">انصراف</button>
        </div>
      </fieldset>
    </form>
  );
