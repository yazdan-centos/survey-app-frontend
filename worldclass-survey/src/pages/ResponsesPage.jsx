...
const buttonClass = 'inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50';
  ...
  return (
    <div dir="rtl" className="mx-auto min-w-0 max-w-6xl space-y-6 px-4 py-8 sm:px-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><h2 className="text-2xl font-bold text-slate-900">پاسخ‌نامه‌ها</h2><p className="mt-2 text-sm text-slate-500">آمار پاسخ‌های ثبت‌شده را مرور کنید و جزئیات هر پاسخ‌نامه را با شناسه آن ببینید.</p></div>
      ...
      <section aria-label="جستجوی پاسخ‌نامه" className="rounded-2xl border border-slate-200 bg-white p-5">
        <form onSubmit={lookup} className="flex flex-wrap items-end gap-3">
          <div className="min-w-0 flex-1 basis-64">
            <label htmlFor="response-id" className="mb-2 block text-sm font-semibold text-slate-700">شناسه پاسخ‌نامه</label>
            <input ref={lookupInput} id="response-id" value={responseId} onChange={(event) => setResponseId(event.target.value)} dir="ltr" required autoComplete="off" spellCheck={false} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" aria-describedby="response-lookup-help" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-700" />
          </div>
        ...
        <p id="response-lookup-help" className="mt-3 text-xs text-slate-500">برای مشاهده پاسخ‌ها و اطلاعات پاسخ‌دهنده، شناسه پاسخ‌نامه ثبت‌شده را وارد کنید.</p>
        {detailError && <p role="alert" className="mt-3 rounded-lg bg-rose-50 p-3 text-sm text-rose-700">{detailError}</p>}
      </section>

      {detailLoading && <p role="status" className="flex items-center justify-center gap-2 rounded-xl bg-white p-8 text-sm text-slate-500"><Loader2 size={18} className="animate-spin" />در حال دریافت پاسخ‌نامه...</p>}
      ...
      {overviewLoading ? <p role="status" className="flex items-center justify-center gap-2 p-8 text-sm text-slate-500"><Loader2 size={18} className="animate-spin" />در حال دریافت آمار...</p> : overview && (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            ].map(([key, label]) => <div key={key} className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold text-primary-800">{responseNumber(overview.summary[key])}</p></div>)}
          </div>
          <section className="min-w-0 space-y-4" aria-labelledby="response-surveys-title">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <label className="min-w-0 text-xs text-slate-600">جستجوی پیمایش<input type="search" value={surveySearch} onChange={(event) => setSurveySearch(event.target.value)} className="mt-1 block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" /></label>
            </div>
            <p className="text-xs text-slate-500">در حال حاضر مرور پاسخ‌نامه‌های فردی با شناسه انجام می‌شود؛ آمار هر پیمایش در زیر آمده است.</p>
            {surveys.map((survey) => <article key={survey.id} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0"><h4 className="break-words font-semibold text-slate-800">{survey.title}</h4><p className="mt-1 text-xs text-slate-500">آخرین پاسخ: {responseDate(survey.lastSubmittedAt)}</p></div>
              ...
              <div className="mt-4 overflow-x-auto">
                <table className="w-full text-right text-sm"><caption className="sr-only">آمار گروه‌های پاسخ‌دهنده {survey.title}</caption>
                  <thead className="bg-slate-50 text-xs text-slate-600"><tr>{['گروه پاسخ‌دهنده', 'پاسخ‌نامه‌ها', 'پاسخ‌های دارای سطح', 'بدون اطلاعات کافی'].map((label) => <th key={label} scope="col" className="whitespace-nowrap px-3 py-3">{label}</th>)}</tr></thead>
                  <tbody className="divide-y divide-slate-100">{(survey.audiences ?? []).map((audience) => <tr key={audience.role}><th scope="row" className="px-3 py-3 font-medium text-slate-700">{responseRoleLabel(audience.role)}</th><td className="px-3 py-3">{responseNumber(audience.responseCount)}</td><td className="px-3 py-3">{responseNumber(audience.answeredCount)}</td><td className="px-3 py-3">{responseNumber(audience.skippedCount)}</td></tr>)}</tbody>
            ...
            {!surveys.length && <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500"><FileText size={28} className="mx-auto mb-3" />{overview.surveys.length ? 'پیمایشی با این نام یافت نشد.' : 'هنوز پیمایشی ثبت نشده است.'}</div>}
          </section>
        </>
      )}
    </div>
